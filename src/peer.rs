...
pub(crate) struct PeerMap {
    ...
    sync_db_cycle: Arc<CycleTaskDb>,
}

unsafe impl Sync for PeerMap {}
...
impl PeerMap {
    pub(crate) async fn new() -> ResultType<Self> {
        ...
        let pm = Self {
            ...
            sync_db_cycle: Arc::new(CycleTaskDb::new()),
        };
        Ok(pm)
    }
    ...
    pub(crate) async fn insert_update_last_reg_time_task(&self, guid: Vec<u8>) -> ResultType<()> {
        let db = self.db.clone();
        #[allow(unused_must_use)]
        self.insert_sync_task(Box::pin(async move {
            match db.update_last_reg_time(guid.clone()).await {
                Ok(_) => {
                    // log::info!("SUCCESS: update_last_reg_time> guid: {:?}", guid);
                }
                Err(e) => {
                    log::error!("ERROR: update_last_reg_time> guid: {:?} e:{:#?}", guid, e);
                }
            }
            Ok(())
        })).await;
    ...
    pub(crate) async fn sync_loop_task(&self) {
        let sync_db_cycle = self.sync_db_cycle.clone();
        let _ = hbb_common::tokio::spawn(async move {
            if let Err(e) = sync_db_cycle.cycle().await {
                log::error!("ERROR:CycleTaskDb::cycle::e{:#?}", e);
            }
        });
    }

    pub(crate) async fn insert_sync_task(&self, task: crate::sync_db::SyncTask) {
        self.sync_db_cycle.insert_sync_task(task).await;
    }

    pub(crate) async fn insert_all_SyncMemToDbTask(&self) -> ResultType<()> {
        let mut vl = Vec::new();
        for (k, v) in self.map.read().await.iter() {
            let v_read = v.read().await;
            vl.push((k.clone(), v_read.guid.clone(), v_read.last_reg_time, v_read.socket_addr));
        }
        let db = self.db.clone();
        #[allow(unused_must_use)]
        self.insert_sync_task(Box::pin(async move {
            for (_id, guid, last_reg_time, _socket_addr) in vl {
                match db.sync_mem_to_db(guid.clone(), crate::datatime_util::instant_to_naive_datetime(last_reg_time)).await {
                    ...
                    Err(e) => {
                        log::error!("ERROR: sync_mem_to_db::update_last_reg_time> guid: {:?} e:{:#?}", guid, e);
                    }
                }
            }
            Ok(())
    ...
    pub(crate) async fn get_or(&self, id: &str) -> LockPeer {
        ...
        if let Some(p) = map_write_lock.get(id) {
            return p.clone();
        }
        let tmp = LockPeer::default();
        map_write_lock.insert(id.to_owned(), tmp.clone());
        tmp
    }
